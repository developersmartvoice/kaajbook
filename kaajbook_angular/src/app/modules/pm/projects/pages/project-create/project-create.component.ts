...
export class ProjectCreateComponent implements OnInit {
    ...
    getCustomTemplateList() {
        this.customTemplateService.getAllTemplates().subscribe(
            (data) => {
                // Create projectTemplateType list
                this.projectTemplateType = data.map((template, index) => ({
                    id: index,
                    name: template.template_name,
                }));

                // Create autoGeneratedTaskNameList
                this.autoGeneratedTaskNameList = data.map((template) => {
                    const tasks = JSON.parse(template.tasks);
                    return Object.keys(tasks); // Extracting only the task names
                });

                this.autoGeneratedTaskNameLogoList = data.map((template) => {
                    const tasks = JSON.parse(template.tasks);
                    return Object.values(tasks); // Extracting only the task names
                });
        ...
        if (selectedTemplate != null) {
            for (
                ...
                this.createAutoGeneratedTaskForm = this.formBuilder.group({
                    ...
                    status: [2, Validators.required],
                    task_logo: [this.autoGeneratedTaskNameLogoList[selectedTemplate][Task]],
